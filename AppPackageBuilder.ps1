...
function New-Package {
    ...
    try {
        ...
        Copy-Item -LiteralPath $Data.MsiPath -Destination (Join-Path $progPath ([IO.Path]::GetFileName($Data.MsiPath)))
        ...
        $install = Set-CommentValue $install 'Registry' ("HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$($Data.ProductCode)")
        ...
        $msiName = ConvertTo-PSDoubleQuotedValue ([IO.Path]::GetFileName($Data.MsiPath))
...
"@
        ...
        $manifest = [ordered]@{
            ...
            SourceMsi = [IO.Path]::GetFileName($Data.MsiPath)
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="Gerador de pacotes SCCM / Intune" Height="820" Width="940"
  ...
  <Grid Margin="18">
    ...
    <StackPanel Grid.Row="0" Margin="0,0,0,12">
      ...
      <TextBlock Text="Selecione um MSI, revise os dados e gere uma cópia preenchida do template." Foreground="#555"/>
    </StackPanel>
    <ScrollViewer Grid.Row="1" VerticalScrollBarVisibility="Auto">
      <Grid>
        ...
        <Label Grid.Row="1" Content="Arquivo MSI *"/><TextBox x:Name="MsiPath" Grid.Row="1" Grid.Column="1" Margin="4" IsReadOnly="True"/><Button x:Name="BrowseMsi" Grid.Row="1" Grid.Column="2" Content="Selecionar..." Padding="14,5" Margin="4"/>
        ...
        <Label Grid.Row="10" Content="ProductCode"/><TextBox x:Name="ProductCode" Grid.Row="10" Grid.Column="1" Margin="4" IsReadOnly="True" Grid.ColumnSpan="2"/>

        <TextBlock Grid.Row="11" Grid.ColumnSpan="3" Text="Instalação e detecção" FontSize="17" FontWeight="SemiBold" Margin="0,14,0,8"/>
        <Label Grid.Row="12" Content="Argumentos MSI"/><TextBox x:Name="InstallArguments" Grid.Row="12" Grid.Column="1" Margin="4" Grid.ColumnSpan="2"/>
        <Label Grid.Row="13" Content="Executável de detecção"/><TextBox x:Name="DetectionPath" Grid.Row="13" Grid.Column="1" Margin="4" Grid.ColumnSpan="2" ToolTip="Opcional. Caminho completo após a instalação; o ProductCode do MSI também pode ser usado para detecção."/>
        <Label Grid.Row="14" Content="Versão de detecção"/><TextBox x:Name="DetectionVersion" Grid.Row="14" Grid.Column="1" Margin="4" Grid.ColumnSpan="2" ToolTip="Opcional. Versão do executável instalado usada em uma regra de detecção por arquivo."/>
        <Label Grid.Row="15" Content="Processos a fechar"/><TextBox x:Name="Processes" Grid.Row="15" Grid.Column="1" Margin="4" Grid.ColumnSpan="2" ToolTip="Separe os nomes por vírgula"/>
        <Label Grid.Row="16" Content="Padrão DisplayName *"/><TextBox x:Name="DisplayPattern" Grid.Row="16" Grid.Column="1" Margin="4" Grid.ColumnSpan="2"/>
        <CheckBox x:Name="RemoveAllVersions" Grid.Row="17" Grid.Column="1" Grid.ColumnSpan="2" Margin="5,10" Content="Remover todas as versões anteriores encontradas pelo DisplayName" IsChecked="True"/>
      </Grid>
    </ScrollViewer>
...
$names = 'MsiPath','OutputRoot','Manufacturer','CompanyToken','AppName','AppToken','Version','SystemCode',
    'Language','InternalVersion','ProductCode','InstallArguments','DetectionPath','DetectionVersion',
    'Processes','DisplayPattern','RemoveAllVersions','BrowseMsi','BrowseOutput','Generate','FolderPreview'
$ui = @{}
...
$ui.BrowseMsi.Add_Click({
    $dialog = New-Object Microsoft.Win32.OpenFileDialog
    $dialog.Filter = 'Windows Installer (*.msi)|*.msi'
    if ($dialog.ShowDialog()) {
        try {
            $ui.MsiPath.Text = $dialog.FileName
            $manufacturer = Get-MsiProperty $dialog.FileName 'Manufacturer'
            $productName = Get-MsiProperty $dialog.FileName 'ProductName'
            $version = Get-MsiProperty $dialog.FileName 'ProductVersion'
            $productCode = Get-MsiProperty $dialog.FileName 'ProductCode'
            $suggestedName = Get-SuggestedApplicationName $productName $version
            $ui.Manufacturer.Text = $manufacturer
            $ui.CompanyToken.Text = ConvertTo-PackageToken $manufacturer
            $ui.AppName.Text = $suggestedName
            $ui.AppToken.Text = ConvertTo-PackageToken $suggestedName
            $ui.Version.Text = $version
            $ui.ProductCode.Text = $productCode
            $ui.DetectionVersion.Text = $version
            $ui.DisplayPattern.Text = $suggestedName
            $ui.Processes.Text = ''
            $ui.InstallArguments.Text = 'INSTALLDIR="C:\Applics\' + (ConvertTo-PackageToken $suggestedName) + '"'
            if ([string]::IsNullOrWhiteSpace($version)) {
                Show-Info 'O MSI não informa ProductVersion. Preencha a versão antes de gerar.'
                $ui.Version.Focus()
            }
        }
        catch { Show-Error "Não foi possível ler o MSI.`n`n$($_.Exception.Message)" }
    }
})

$ui.Generate.Add_Click({
    $required = [ordered]@{
        'Arquivo MSI' = $ui.MsiPath.Text; 'Pasta de saída' = $ui.OutputRoot.Text
        ...
        'Versão interna' = $ui.InternalVersion.Text; 'ProductCode' = $ui.ProductCode.Text
        'Padrão DisplayName' = $ui.DisplayPattern.Text
    }
    ...
    if (-not (Test-Path -LiteralPath $ui.MsiPath.Text -PathType Leaf)) { Show-Error 'O arquivo MSI não existe.'; return }
    ...
    try {
        ...
        $data = @{
            MsiPath = $ui.MsiPath.Text; OutputRoot = $ui.OutputRoot.Text
